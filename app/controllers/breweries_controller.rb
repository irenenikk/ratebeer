class BreweriesController < ApplicationController
  ...
  def index
    ...
    @active_breweries = case @order
      when 'name' then
        if session[:last_name_order] == 'ASC'
          session[:last_name_order] = 'DESC'
          @active_breweries.sort_by{ |b| b.name }.reverse!
        else
          session[:last_name_order] = 'ASC'
          @active_breweries.sort_by{ |b| b.name }
        end
      when 'year' then
        if session[:last_year_order] == 'ASC'
          session[:last_year_order] = 'DESC'
          @active_breweries.sort_by{ |b| b.year }.reverse!
        else
          session[:last_year_order] = 'ASC'
